...
export default function Admin() {
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
    return localStorage.getItem('hbbr_admin_auth') === 'true';
  });
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        localStorage.setItem('hbbr_admin_auth', 'true');
        setAuthError('');
        loadReservations();
      } else {
        setAuthError(
  ...
  const handleDelete = async (id: number, name: string) => {
    if (!window.confirm(`[${name}] 님의 예약 내역을 정말 삭제하시겠습니까?`)) {
      return;
    }
    try {
      const res = await fetch(`/api/reservations/${id}`, {
  ...
  return (
    <div className="min-h-screen bg-brand-beige/20 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Search & Filter Bar */}
        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-brand-green/20 shadow-sm mb-6 flex flex-col md:flex-row gap-4 items-center justify-between">
                      ...
                      <tr key={r.id} className="hover:bg-brand-beige/10 transition-colors">
                        ...
                        <td className="py-4 px-6 whitespace-nowrap text-right">
                          <button
                            ...
                            className="p-1.5 text-brand-brown/40 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"
                            title="예약 내역 삭제"
                          >
