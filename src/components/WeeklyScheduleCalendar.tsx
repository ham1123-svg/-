...
export default function WeeklyScheduleCalendar({
  ...
  const handleSlotClick = (dateStr: string, timeStr: string, isClickable: boolean, dayName: string) => {
    ...
    // If formRef is provided, gently scroll to it
    if (formRef?.current) {
      setTimeout(() => {
  ...
  return (
    <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xl border border-brand-green/20 mb-12">
      {/* Calendar Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 mb-6 border-b border-brand-green/20">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-xs text-brand-brown/60">
              희망하는 시간대를 클릭하면 예약 폼에 바로 자동 적용됩니다.
            </span>
          </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 sm:px-3 sm:py-2 rounded-xl border border-brand-green/30 text-brand-brown text-xs font-bold hover:bg-brand-beige/40 disabled:opacity-40 disabled:cursor-not-allowed transition-all flex items-center gap-1"
            title="이전 주"
          >
          ...
          <button
            ...
            className="p-2 sm:px-3 sm:py-2 rounded-xl border border-brand-green/30 text-brand-brown text-xs font-bold hover:bg-brand-beige/40 transition-all flex items-center gap-1"
            title="다음 주"
          >
          ...
          {weekOffset > 0 && (
            <button
              ...
              className="px-2.5 py-2 bg-white border border-brand-green/30 hover:border-brand-sage text-brand-sage text-xs font-bold rounded-xl transition-all"
              title="이번 주로 돌아가기"
            >
              오늘
            </button>
      ...
      <AnimatePresence>
        {selectedDate && selectedTime && (
          ...
          >
            <div className="p-3.5 bg-brand-green/20 border border-brand-sage/40 rounded-2xl flex items-center justify-between gap-3 text-xs text-brand-brown">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 bg-brand-sage text-white rounded-full flex items-center justify-center shrink-0">
                  <Check className="w-3.5 h-3.5 stroke-[3]" />
                </div>
                <div>
                  <span className="font-bold text-brand-brown">선택된 상담 일정:</span>{' '}
                  <strong className="text-brand-sage font-extrabold text-sm">{selectedDate} ({weekDays.find(d => d.dateStr === selectedDate)?.dayName || ''}) {selectedTime}</strong>
                  <span className="text-brand-brown/60 ml-2">
                    (아래 예약 신청 폼에 자동으로 반영되었습니다)
                  </span>
                </div>
              </div>
              <button
                ...
                className="px-3 py-1.5 bg-brand-sage text-white font-bold rounded-xl hover:bg-brand-sage/90 transition-all shrink-0 text-xs shadow-xs"
              >
                예약 정보 입력하기 ↓
              </button>
            </div>
              ...
              <div className="flex-1 space-y-1.5">
                {day.isSunday ? (
                  ...
                  filteredSlots.map((timeStr) => {
                    ...
                    const isSelected = selectedDate === day.dateStr && selectedTime === timeStr;
                      ...
                      <button
                        ...
                        title={
                          isSelected 
                            ? '선택된 시간' 
                            : isAvailable 
                            ? `${day.dateStr} [${TIME_SLOT_DETAILS[timeStr]?.session || ''}] ${timeStr} 예약하기` 
                            : isBlocked
